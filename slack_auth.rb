...
BOT_SCOPE = 'bot,chat:write:bot,reactions:read,reactions:write'
...
class SlackAuth < Sinatra::Base
  ...
  get '/install' do
    erb :slack_auth, :locals => {bot_scope: BOT_SCOPE, client_id: SLACK_CONFIG[:slack_client_id], redirect_uri: 'https://'+ENV['HOST']+'/install/finish'}
  end
  ...
  get '/install/finish' do
    ...
    begin
      ...
      client.chat_postMessage(channel: installer, as_user:true, text: "Hello <@#{installer}>! Thanks for installing me. Just invite me into any channel, and I will walk you through setting me up to start tweeting links.")
